...
class ReviewService {
  Future<List<Review>> fetchReviews(String isbn) async {
    final response = await http.get(
      Uri.parse('http://13.210.68.246:8080/api/review/get/by-isbn/$isbn'),
      headers: {
  ...
  Future<void> addReview(int loanId, String reviewText, int reviewScore) async {
    final response = await http.post(
      Uri.parse('http://13.210.68.246:8080/api/review/add'),
      headers: {
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: json.encode({
...
class ConditionService {
  Future<List<ReviewConditionBox>> fetchConditions(String bookId) async {
    final response = await http.get(
      Uri.parse('http://13.210.68.246:8080/reviewCondition/list/$bookId'),
      headers: {
        'Content-Type': 'application/json; charset=UTF-8',
      },
    );

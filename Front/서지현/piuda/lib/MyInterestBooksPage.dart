...
class _MyInterestBooksPageState extends State<MyInterestBooksPage> {
  ...
  Future<void> loadInterestBooks() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (UserInterestBook interestBook in interestBooksData) {
          String imageUrl = await BookUtils.fetchBookCover(interestBook.book.bookIsbn);
          interestBook.book.imageUrl = imageUrl;
        }
        ...
        for (int i = 0; i < interestBooksData.length; i++) {
          await BookUtils.fetchBookCover(interestBooksData[i].book.bookIsbn);
        }
      } else {
  ...
  Widget buildInterestBookItem(int index) {
    if (interestBooks[index].book.imageUrl.isEmpty) {
      BookUtils.fetchBookCover(interestBooks[index].book.bookIsbn);
    }
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => BookDetail(book_id: interestBooks[index].book.book_id)
            )
        );
      },
      child: Container(
